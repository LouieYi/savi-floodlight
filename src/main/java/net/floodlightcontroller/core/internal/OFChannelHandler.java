...
class OFChannelHandler extends SimpleChannelInboundHandler<Iterable<OFMessage>> {
	 ...
	 * the OFChannelHandler we don't need atomic. We will count down
	 */
	...
	public abstract class OFChannelState {
		...
		void logState() {
			log.debug("{} OFConnection Handshake - enter state {}", getConnectionInfoString(),
	...
	/**
	 * We send a HELLO to the switch and wait for a reply. Once we receive the
	 * reply we send an OFFeaturesRequest Next state is WaitFeaturesReplyState
	 */
	class WaitHelloState extends OFChannelState {
		...
		void processOFHello(OFHello m) throws IOException {
			...
			/*
			 * First, check if there's a version bitmap supplied. WE WILL ALWAYS
			 * HAVE a controller-provided version bitmap.
			 */
			if (theirVersion.compareTo(OFVersion.OF_13) >= 0 && !m.getElements().isEmpty()) {
				...
				if (commonVersion == null) {
					...
					log.info("Negotiated OpenFlow version of {} for {} with greatest version bitmap algorithm.",
							commonVersion.toString(), channel.remoteAddress());
			...
			/*
			 * If there's not a bitmap present, choose the lower of the two
			 * supported versions.
			 */
			else if (theirVersion.compareTo(factory.getVersion()) < 0) {
				log.info("Negotiated down to switch OpenFlow version of {} for {} using lesser hello header algorithm.",
						theirVersion.toString(), channel.remoteAddress());
				factory = OFFactories.getFactory(theirVersion);
				OFMessageDecoder decoder = pipeline.get(OFMessageDecoder.class);
				decoder.setVersion(theirVersion);
			} /*
				 * else The controller's version is < or = the switch's, so keep
				 * original controller factory.
				 */
			else if (theirVersion.equals(factory.getVersion())) {
				log.info("Negotiated equal OpenFlow version of {} for {} using lesser hello header algorithm.",
						factory.getVersion().toString(), channel.remoteAddress());
			} else {
				log.info(
	...
	/**
	 * We are waiting for a features reply message. Once we receive it we send
	 * capture the features reply. Next state is CompleteState
	 */
	class WaitFeaturesReplyState extends OFChannelState {
		...
		void processOFHello(OFHello m) throws IOException {
			/*
			 * Brocade switches send a second hello after the controller
			 * responds with its hello. This might be to confirm the protocol
			 * version used, but isn't defined in the OF specification.
			 * 
			 * We will ignore such hello messages assuming the version of the
			 * hello is correct according to the algorithm in the spec.
			 * 
			 * TODO Brocade also sets the XID of this second hello as the same
			 * XID the controller used. Checking for this might help to assure
			 * we're really dealing with the situation we think we are.
			 */
			if (m.getVersion().equals(factory.getVersion())) {
				log.warn("Ignoring second hello from {} in state {}. Might be a Brocade.", channel.remoteAddress(),
						state.toString());
			} else {
				super.processOFHello(
						m); /* Versions don't match as they should; abort */
			}
		}
		...
		void processOFMessage(OFMessage m) throws IOException {
			if (m.getType().equals(OFType.PACKET_IN)) {
				log.warn("Ignoring PACKET_IN message from {} during OpenFlow channel establishment.",
						channel.remoteAddress());
			} else {
	...
	/**
	 * This state denotes that the channel handshaking is complete. An OF
	 * connection is generated and passed to the switch manager for handling.
	 */
	class CompleteState extends OFChannelState {
		...
		void enterState() throws IOException {
			...
			if (featuresReply.getVersion().compareTo(OFVersion.OF_13) < 0) {
				connection = new OFConnection(featuresReply.getDatapathId(), factory, channel, OFAuxId.MAIN,
						debugCounters, timer);
			}
			// Handle 1.3 connections
			else {
				connection = new OFConnection(featuresReply.getDatapathId(), factory, channel,
						featuresReply.getAuxiliaryId(), debugCounters, timer);
	 ...
	 * @param debugCounters
	 */
	OFChannelHandler(@Nonnull IOFSwitchManager switchManager, @Nonnull INewOFConnectionListener newConnectionListener,
			@Nonnull ChannelPipeline pipeline, @Nonnull IDebugCounterService debugCounters, @Nonnull Timer timer,
			@Nonnull List<U32> ofBitmaps, @Nonnull OFFactory defaultFactory) {
	...
	private OFVersion computeOFVersionFromBitmap(List<U32> theirs) {
		...
		while (theirsItr.hasNext() && oursItr.hasNext()) {
			...
			for (int i = 0; i < size; i++) { /*
												 ...
												 */
				int tmp = common
						& (1 << i); /* Select the bit of interest, 0-31 */
				if (tmp != 0) { /* Is the version of this bit in common? */
					for (OFVersion v : OFVersion
							.values()) { /*
											 * Which version does this bit
											 * represent?
											 */
			...
			pos++; /*
					 * OFVersion position. 1-31 = 1, 32 - 63 = 2, etc. Inc at end
					 * so it starts at 0.
					 */
		}
		return version;
	...
	public void channelInactive(ChannelHandlerContext ctx) {
		...
		if (this.connection != null) {
			// Alert the connection object that the channel has been
			// disconnected
	...
	public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
		if (cause instanceof ReadTimeoutException) {
			...
			if (featuresReply.getVersion().compareTo(OFVersion.OF_13) < 0) {
				log.error("Disconnecting switch {} due to read timeout on main cxn.", getConnectionInfoString());
				ctx.channel().close();
			} else {
				if (featuresReply.getAuxiliaryId().equals(OFAuxId.MAIN)) {
					log.error("Disconnecting switch {} due to read timeout on main cxn.", getConnectionInfoString());
			...
			log.error("Disconnecting switch {}: failed to complete handshake. Channel handshake complete : {}",
					getConnectionInfoString(), this.state.channelHandshakeComplete);
			...
			log.error("Disconnecting switch {} due to IO Error: {}", getConnectionInfoString(), cause.getMessage());
			if (log.isDebugEnabled()) {
			...
			log.error("Disconnecting switch {} due to switch state error: {}", getConnectionInfoString(),
					cause.getMessage());
			if (log.isDebugEnabled()) {
				// still print stack trace if debug is enabled
				log.debug("StackTrace for previous Exception: ", cause);
			...
			log.error("Disconnecting switch {} due to OF Aux error: {}", getConnectionInfoString(), cause.getMessage());
			if (log.isDebugEnabled()) {
				// still print stack trace if debug is enabled
				log.debug("StackTrace for previous Exception: ", cause);
			}
			...
			log.error("Disconnecting switch " + getConnectionInfoString() + " due to message parse failure", cause);
			...
			log.error("Error while processing message from switch " + getConnectionInfoString() + "state " + this.state,
					cause);
	...
	private void setAuxChannelIdle() {
		IdleStateHandler idleHandler = new IdleStateHandler(PipelineIdleReadTimeout.AUX, PipelineIdleWriteTimeout.AUX,
				0);
	...
	private void setSwitchHandshakeTimeout() {
		...
		HandshakeTimeoutHandler handler = new HandshakeTimeoutHandler(this, this.timer,
				PipelineHandshakeTimeout.SWITCH);

		pipeline.replace(PipelineHandler.CHANNEL_HANDSHAKE_TIMEOUT, PipelineHandler.SWITCH_HANDSHAKE_TIMEOUT, handler);
	}
	...
	private void sendHelloMessage() throws IOException {
		...
		/*
		 * Our highest-configured OFVersion does support version bitmaps, so
		 * include it
		 */
